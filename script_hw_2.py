...
print(f"total number of plates: {len(plates_list)}")
new_list = set(plates_list)
print(f"total number of unique plates: {len(new_list)}")
car_number = input('enter plate letters and numbers: ')
car_number = car_number.upper()


def check():
    """
    Function of the checkin - if entering plate exists in our database
    :return: true or false
    """
    if car_number in new_list:
        return f'Thank you, the plate is exist in our database: {car_number}'
    else:
...
def length_check():
    if car_number in new_list:
        spisok = list(car_number)
        spisok = spisok[2:6]
        nums_list = [int(s) for s in spisok]
        ...
        def sum(nums_list):
            ...
            for element in nums_list:
                summa = summa + element
            return summa
        return f'Total sum of numbers in the plate is: {sum(nums_list)}'
    elif len(car_number) < 8:
        return 'False: you entered few letters and numbers'
    elif len(car_number) > 8:
        return 'False: you entered a lot of letters and numbers'
    else:
        return 'False: plate does not exist'
